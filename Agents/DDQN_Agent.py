...
class DQNAgent:
    ...
    specific states by updating the Q Values in each state. We then use the
    quality of the available actions to determine the overall Q Values in the state.
    The agent also has a target model. We use the main model to determine the
    correct action for the nextState and use the target model to determine
    the action's value. This allows our model to converge with more stability
    ...
    def updateModel(self, memoryBuffer):
        ...
        targetPredictNext = self.targetModel.forward(nextStates)
